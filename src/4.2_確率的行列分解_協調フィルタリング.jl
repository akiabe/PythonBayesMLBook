...
# ╔═╡ 20ae6ac8-e899-47f3-91f3-d70b5ee2c58a
using DataFrames

# ╔═╡ 784e9a6b-da74-4aff-a1d2-ac3c34d6d87a
using CSV

# ╔═╡ b3cbaae3-d129-4bda-a830-4a49f748e8d2
using Plots

# ╔═╡ 034618ed-ecf0-4350-b20a-bc27cada7c12
using Statistics

# ╔═╡ 6b1a5c7e-17e8-4697-a04c-fa9b331311f3
using Turing

# ╔═╡ 4c3944e0-e254-40fd-be9b-8d70e3372784
md"""
Data
"""

# ╔═╡ 9442e5c6-89a0-4df0-922c-91fc6ed93976
__data = CSV.read("u.data", DataFrame, header=false);

# ╔═╡ c540a0ad-e85d-49cb-9145-4187704d608b
_data = __data[:, 1:3];

# ╔═╡ c3bfebb3-c7df-4253-a527-87e92aa57324
data = rename(_data, :Column1 => :userid, :Column2 => :itemid, :Column3 => :rating,)

# ╔═╡ 44afbbfb-4a65-4b79-8ba2-29be4163a3f8
histogram(data[:, 3], bins=10, xlabel="rating", label=:none)

# ╔═╡ ecb046c1-ffc0-4510-abd0-125b36af8e45
dense_data = unstack(data, :userid, :itemid, :rating)

# ╔═╡ 029b8f19-52c2-4c1a-a7b7-660da03046fd
describe(data[:, 3])

# ╔═╡ 8a2a9539-2a8c-4dd4-b08a-ff1ff358b626
item_mean = combine(groupby(data, :itemid), :rating => mean)

# ╔═╡ 97fc5c4c-6a07-4dcf-97a8-e0ee5bcf7fe5
item50 = item_mean[1:50, :]

# ╔═╡ 0e33d731-ec64-4f13-9d64-7fda5caf89a5
bar(item50[:, 1], item50[:, 2], label=:none, xlabel="itemid", ylabel="rating_mean", title="mean ratings for 50 movies")

# ╔═╡ 097b20a8-9940-4297-8a2a-91c52efdcce8
sort_item_mean = sort!(item_mean, :rating_mean, rev=true)

# ╔═╡ ade7557a-2efe-4b15-a431-272998f561fb
sort_item_mean[begin:10, :]

# ╔═╡ 7b938aef-4e9b-48bc-92f7-558dcb3eee94
sort_item_mean[end-10:end, :]

# ╔═╡ 714542bc-a0cc-45d6-9642-c7df28480a18
user_mean = sort!(combine(groupby(data, :userid), :rating => mean), :rating_mean)

# ╔═╡ 2c11c2e3-dcdf-47e2-bd7c-67ca4f3a44c8
md"""
Training data vs Test data
"""

# ╔═╡ e6273d50-4d3f-45f1-8ded-c5b27034b2d7
N, M = size(dense_data)

# ╔═╡ 99b6fa52-bd4d-4247-8772-180e82f21512
train = dense_data[begin:700, :];

# ╔═╡ c6f133bf-e90b-472b-803b-bb49d3a43619
test = dense_data[200:end, :];

# ╔═╡ aa052549-331e-437c-898f-9323c864c88b
md"""
Probabilistic Matrix Factorization
"""

# ╔═╡ c908ec0e-5b08-4d5d-8d43-1ba8e4dc66c1
train

# ╔═╡ 55d58fb0-7313-4d29-86bc-b3aaf7048f87
mat_train = Matrix(train)[:, 2:end]

# ╔═╡ 669c000b-f52d-4923-ac65-d63b754006bc
function f(data)
	N, M = size(data)
	_αᵤₛₑᵣ = zeros(N)
	_αᵢₜₑₘ = zeros(M)
	for n in 1:N
		_αᵤₛₑᵣ[n] = var(skipmissing(data[n, :]))
	end
	αᵤₛₑᵣ = mean(filter(!isnan, _αᵤₛₑᵣ))

	for m in 1:M
		_αᵢₜₑₘ[m] = var(skipmissing(data[:, m]))
	end
	αᵢₜₑₘ = mean(filter(!isnan, _αᵢₜₑₘ))
	
	return αᵤₛₑᵣ, αᵢₜₑₘ
end

# ╔═╡ 393dd236-5df5-4a03-82d0-8913d63c2b96
αᵤₛₑᵣ, αᵢₜₑₘ = f(mat_train);

# ╔═╡ 21017b0a-3ecd-4ea4-aee3-692f3a37717e
αᵤₛₑᵣ

# ╔═╡ 7664877f-c2b1-4ad1-bbe7-cd1610154c85
αᵢₜₑₘ

# ╔═╡ eba2c0cf-fb46-42ee-8613-383cd41189cf


# ╔═╡ 81dbe7e0-2e47-4b0a-99fb-c06dd6028bda


# ╔═╡ 99ccbe79-4ad3-48b2-878f-621de60af6dd
