...
# ╔═╡ de4db81e-ff2a-11ec-033a-19eabbad7d1a
using Distributions; using Plots; using Turing; using StatsPlots

# ╔═╡ 332478c9-e603-459b-ab0b-5edb4be7b671
begin
	μ₁ = [
	    -1.0
	    1.0
...
# ╔═╡ faabdcd6-a674-4b79-876c-0db0e42973b4
HalfCauchy(μ, σ) = Truncated(Cauchy(μ, σ), 0.0, Inf)

# ╔═╡ 1fd05ccc-e36a-4118-8665-0c351e84267c
...
# ╔═╡ 73cc6e05-46b2-4c01-ab97-f161bbb1384a
model = gaussian_mixture_model(X)

# ╔═╡ 003845f3-fc29-4f8a-9606-c4a211ab3481
# ╠═╡ show_logs = false
chain = sample(model, PG(15), 1000);

# ╔═╡ 34e2e1e0-b465-40c0-93e2-f01dc34d8365
display(chain)

# ╔═╡ 6fab5f77-7a09-4165-b719-211739a07d1f
describe(chain)

# ╔═╡ 3517fded-1263-47b0-8ce5-6bc418b971f8
plot(chain[["μ[1]", "μ[2]"]])

# ╔═╡ 7693331c-379d-4ce2-9a78-999ac0f5c10f
begin
	x₁s = range(-3, 3, length=100)
	x₂s = range(-3, 3, length=100)
	μ_mean = [mean(chain, "μ[$i]") for i in 1:2]
	σ_mean = [mean(chain, "σ[$i]") for i in 1:2]
	z_mean = [mean(chain, "z[$i]") for i in 1:2]
end

# ╔═╡ 20fac1d6-bc01-4be6-af09-df669f5962ff
μ_mean

# ╔═╡ cf9de21d-d295-4594-a61b-12dc14391707
σ_mean

# ╔═╡ a9d5246f-afae-42ef-b45b-59ad95c68e0f
z_mean

# ╔═╡ 4b000ba0-4df8-4920-a239-2ecceec6836f
