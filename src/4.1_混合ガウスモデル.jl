...
# ╔═╡ 049d04bc-ffe9-11ec-33d0-f7363c0ef205
begin
	using Distributions
	using Plots
	using Turing
...
# ╔═╡ 44683249-9765-4d47-b3b4-8294e0401734
md"""
真のモデル
"""

# ╔═╡ 163b8466-1466-4bed-885e-b6beb4249e47
begin
	D = 2
	K = 3
	...
	true_model = MixtureModel(gauss_true, π_true)
end

# ╔═╡ b9a9ee80-f7c7-4dd9-8a04-afd5b2372fa8
contour(-50:70, -60:80, (x, y) -> pdf(true_model, [x, y]), title="True model", xlabel="x1", ylabel="x2")

# ╔═╡ 37aa7aa4-c6b2-4e3e-b15f-303090469857
md"""
観測データの生成
"""

# ╔═╡ c6581c82-b614-4b5c-bfd2-6639a4cd5021
begin
	N = 100
	...
	for n in 1:N
	    k = cluster_num[n]
	    gauss_obs = MvNormal(μ_obs[k], Σ_obs[k])
	    data[:, n] = rand(gauss_obs)
	end
...
# ╔═╡ 15c3d282-3c7a-4839-a5d9-c1f07d5b5e29
begin
	each_cluster = [zeros(D, N), zeros(D, N), zeros(D, N)]
	for n in 1:N
	    k = cluster_num[n]
	    gauss_obs = MvNormal(μ_obs[k], Σ_obs[k])
	    each_cluster[k][:, n] = rand(gauss_obs) 
	end
...
# ╔═╡ bf9f0e52-986a-4c92-8104-30caa6998d46
md"""
サンプリング
"""

# ╔═╡ f8fec2b4-72e4-4eea-af57-63705b05e8b9
@model function gaussian_mixture_model(x)
	...
	for n in 1:N
		k[n] ~ distribution_assignments
		x[:, n] ~ distribution_clusters[k[n]]
	end
	return k
end

# ╔═╡ bcac40f0-0e24-495a-affd-bdb8bd523a8f
model = gaussian_mixture_model(data);

# ╔═╡ b53f377a-4e42-4d85-90e5-f1cf1f171a93
begin
...
# ╔═╡ d0af16da-3434-4fc9-91dc-9452711bc43e
plot(chains[["μ[1]", "μ[2]", "μ[3]"]]; colordim=:parameter, legend=true)

# ╔═╡ 1b19c6ff-85de-4590-b957-71b74ad49449
plot(chains[["w[1]", "w[2]", "w[3]"]]; colordim=:parameter, legend=true)

# ╔═╡ 29f4cd12-7b63-4c1d-9fa9-e8062fa848a4
chain = chains[:, :, 1];

# ╔═╡ 64ceb0da-503b-4fe0-9485-a0c1f33d81c4
begin
	μ_mean = [mean(chain, "μ[$i]") for i in 1:3]
	w_mean = [mean(chain, "w[$i]") for i in 1:3]
end

# ╔═╡ 98a340dd-5c0e-410c-95ed-731ad020a2e2
mixturemodel_mean = MixtureModel([MvNormal(Fill(μₖ, 2), I) for μₖ in μ_mean], w_mean)

# ╔═╡ 8919b894-83e3-4584-b7d5-c3634d2a6e6a
begin
	contour(-50:70, -60:80, (x, y) -> pdf(mixturemodel_mean, [x, y]), xlabel="x1", ylabel="x2", label=:none)
	scatter!(data[1, :], data[2, :], label=:none)
end
