...
# ╔═╡ 88215c4c-a2c6-4ad5-b949-2f3ef6dc71b8
begin
	using Distributions
	using Plots
end

# ╔═╡ 90e32a91-9b81-47f0-a41e-fe9a85db12e6
using Turing

# ╔═╡ 4dca1260-8835-48f6-98d7-0dfc0bf2b4bb
md"""
ベイジアン主成分分析
"""

# ╔═╡ 3641bb2a-2fcd-44d7-b596-39698aac3f0c
md"""
データの生成・可視化
"""

# ╔═╡ 51a18229-662d-41db-aa64-5f8ed49dc680
function build_toy_dataset(N, D, K, σ)
	x = zeros(D, N)
	w = rand(Normal(), D, K)
	z = rand(Normal(), K, N)
	μ = w * z
	for d in 1:D
		for n in 1:N
			x[d, n] = rand(Normal(μ[d, n], σ))
		end
	end
	return x
end;

# ╔═╡ ec69153c-a28d-41a5-a2a1-329fd34af131
N = 200; D = 2; K = 1; σ = 0.8;

# ╔═╡ 553f99d6-945c-45cd-a954-6826fc64e128
data = build_toy_dataset(N, D, K, σ)

# ╔═╡ 198df274-451f-4c9b-b5c7-91b4e1dd4148
scatter(data[1, :], data[2, :], xlabel="x1", ylabel="x2", label=:none, title="Simulated dataset")

# ╔═╡ ecada296-b15d-4e2a-bbc5-45e071a19b7d
md"""
モデル
"""

# ╔═╡ 2e442c3b-75cd-4213-a641-a74962d04553
Z = rand(Normal(), K, N)

# ╔═╡ 6ceefd48-6582-4d41-89a2-faccdc4e00d7
W = rand(Normal(), D, K)

# ╔═╡ 0d3252d2-4f32-4bcd-b747-ecc0dbc7374c
WZ = W * Z

# ╔═╡ 27003328-ca74-4b80-97f1-2993712026dd
HalfCauchy(μ, σ) = Truncated(Cauchy(μ, σ), 0.0, Inf)

# ╔═╡ 22e9d3f8-0474-4955-b889-63f68b07c424
σI = rand(HalfCauchy(0.0, 5.0))

# ╔═╡ afb15f8a-7ead-449c-ab6f-757329044b23
 x = zeros(D, N)

# ╔═╡ 9759d3ca-1ca8-479a-86c3-4b026dda000a
for d in 1:D
	for n in 1:N
		x[d, n] = rand(Normal(WZ[d, n], σI))
	end
end

# ╔═╡ 32bb6210-f8ad-4cb5-8851-e1f5684e2e9b
x

# ╔═╡ 8cbe10ee-f789-46f7-b007-7a8919c23a73
scatter(x[1, :], x[2, :], xlabel="x1", ylabel="x2", label=:none)

# ╔═╡ 6f533803-794b-4f95-93a9-787e30b9b9d6
md"""
MCMC
"""

# ╔═╡ 9409da63-5790-4a14-b2a7-bbb4aa579b10
@model function bayesian_pca(data)
	D, N = size(data)
	Z = zeros(1, N)
	W = zeros(D, 1)
	X = zeros(D, N)
	...
	σ ~ HalfCauchy(0.0, 5.0)

	for d in 1:D
		for n in 1:N
			X[d, n] ~ Normal(W[d] * Z[n], σ)
		end
	end
...
# ╔═╡ 66600018-dd0b-4440-8ea2-f425e1a5bc13
model = bayesian_pca(data);

# ╔═╡ 24d7fb00-c627-4ffa-a100-31be4e3bdb51
chain = sample(model, NUTS(), 3000);

# ╔═╡ 1d57de2e-be57-491b-809b-64e06195db4a
params = get_params(chain)

# ╔═╡ fe83e3ec-359d-4274-9841-fc9a6cff9b60
keys(params)

# ╔═╡ 486e4eb7-318c-4cf0-ba65-66f99e607695
md"""
可視化
"""

# ╔═╡ a9266ce5-b9eb-436d-954c-918328217adf
w_mean = mean(group(chain, :W))[:, 2]

# ╔═╡ 34bb6f03-d9cd-4f82-8a43-6cd0afa77361
size(w_mean)

# ╔═╡ 334597dc-b05c-413b-a406-0a5b883787ec
z_mean = mean(group(chain, :Z))[:, 2]

# ╔═╡ 63f80f7f-7e23-4c22-a6d5-8c7f5fb2c21b
size(z_mean)

# ╔═╡ 608d09dd-c200-48dc-a840-7b9e0a9a015b
σ_mean = mean(params[:σ])

# ╔═╡ 77cd244a-e616-4131-a771-2ec4c105a3c1
pred = zeros(D, N)

# ╔═╡ 21320a18-b801-45fb-8ad0-dcd25badc385
for d in 1:D
	for n in 1:N
		pred[d, n] = rand(Normal(w_mean[d] * z_mean[n], σ_mean))
	end
end

# ╔═╡ b80ac44a-a0b5-4cd8-8ce8-147f2f8cc214
pred

# ╔═╡ 6430b4ea-ee92-454b-aadc-9db21c00b509
begin
	scatter(x[1, :], x[2, :], xlabel="x1", ylabel="x2", label="train data")
	scatter!(pred[1, :], pred[2, :], label="pred")
end
