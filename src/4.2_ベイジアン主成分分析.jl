...
# ╔═╡ ba5ccba5-feee-4726-91cb-fcaef25adaf4
begin
	using Distributions
	using Plots
	using Random
...
# ╔═╡ 5a0bcdee-bfd2-4521-b50d-698d7dfcc5be
function build_toy_dataset(N, D, K, σ)
	x_train = zeros(D, N)
	w = rand(Normal(0.0, 1.0), (D, K))
	z = rand(Normal(0.0, 1.0), (K, N))
	mean = w * z
	for d in 1:D
		for n in 1:N
			x_train[d, n] = rand(Normal(mean[d, n], σ))
		end
	end
	return x_train
end

# ╔═╡ 862ce6af-8a68-4317-962d-d42a6906cbeb
begin
	N = 500
	D = 2
	K = 1
	...
	x_train = build_toy_dataset(N, D, K, σ)
	
	scatter(x_train[1, :], x_train[2, :], xlabel="x1", ylabel="x2", label=:none, title="Simulated dataset")
end

# ╔═╡ e2269223-5fd3-4816-9311-bf4d6e27b061


# ╔═╡ 0d92ad31-bcc7-42f8-8b01-bf35d53812e7


# ╔═╡ b2e55903-eb79-472b-af4c-f546ab8a3964


# ╔═╡ 04ada7cd-1e3f-4a46-a714-932f2e3b2b33


# ╔═╡ e7b6729b-d4ad-4fe2-a7ae-0883395c0ee8
